...
from lavis.models.blip2_models.qmod_dsf import VideoEvidenceAdapter


@registry.register_model("blip2_vicuna_instruct")
class Blip2VicunaInstructDSF(Blip2Base):
    ...
    def __init__(
        ...
        self.video_evidence_adapter = VideoEvidenceAdapter(
            hidden_size=self.Qformer.config.hidden_size,
            max_num_frames=max_num_frames,
            temporal_layers=temporal_layers,
            mod_keep_ratio=mod_keep_ratio,
            mod_expansion=mod_expansion,
            dropout=mod_dropout,
            dsf_steps=dsf_steps,
            dsf_init_noise_scale=dsf_init_noise,
            dsf_target_temperature=dsf_target_temp,
        )
        self.lambda_mod = lambda_mod
        self.lambda_dsf = lambda_dsf
    ...
    def forward(self, samples):
        ...
        if is_video:
            if self.use_memory_bank and self.qformer_text_input:
                frame_query_tokens = torch.stack(frame_query_tokens, dim=1)
                question_tokens = query_output.last_hidden_state[
                    :, query_tokens.size(1):, :
                ]
                question_mask = text_Qformer.attention_mask
            else:
                query_hidden = query_output.last_hidden_state.reshape(
                    B, T, query_output.last_hidden_state.size(1), -1
                )
                frame_query_tokens = query_hidden[
                    :, :, :query_tokens.size(1), :
                ]
                if self.qformer_text_input:
                    question_tokens = query_hidden[
                        :, 0, query_tokens.size(1):, :
                    ]
                    question_mask = text_Qformer.attention_mask.reshape(
                        B, T, -1
                    )[:, 0]
                else:
                    question_tokens = frame_query_tokens.mean(dim=1)
                    question_mask = torch.ones(
                        question_tokens.shape[:2],
                        dtype=torch.long,
                        device=image.device,
                    )
            evidence_output = self.video_evidence_adapter(
                frame_query_tokens,
                question_tokens,
                question_mask,
                training=self.training,
            )
            inputs_llm = self.llm_proj(evidence_output["llm_visual_tokens"])
        else:
        ...
        if evidence_output is None:
            mod_aux_loss = answer_loss.new_zeros(())
            dsf_loss = answer_loss.new_zeros(())
        else:
            mod_aux_loss = evidence_output["mod_aux_loss"]
            dsf_loss = evidence_output["dsf_loss"]
        loss = (
            answer_loss
            + self.lambda_mod * mod_aux_loss
            + self.lambda_dsf * dsf_loss
        )

        return {
            "loss": loss,
            "answer_loss": answer_loss.detach(),
            "mod_aux_loss": mod_aux_loss.detach(),
            "dsf_loss": dsf_loss.detach(),
        }
    ...
    def from_config(cls, cfg):
        ...
        mod_keep_ratio = cfg.get("mod_keep_ratio", 0.5)
        mod_expansion = cfg.get("mod_expansion", 2)
        mod_dropout = cfg.get("mod_dropout", 0.1)
        lambda_mod = cfg.get("lambda_mod", 0.01)
        lambda_dsf = cfg.get("lambda_dsf", 1.0)
        dsf_steps = cfg.get("dsf_steps", 20)
        dsf_init_noise = cfg.get("dsf_init_noise", 0.1)
        dsf_target_temp = cfg.get("dsf_target_temp", 0.07)
        ...
        model = cls(
            ...
            lambda_dsf=lambda_dsf,
            dsf_steps=dsf_steps,
            dsf_init_noise=dsf_init_noise,
            dsf_target_temp=dsf_target_temp,
        )
